...
## Modèles de classification et d'étiquettage

Jusqu'à présent, nous avons étudié l'utilisation de la vision artificielle pour créer un modèle classant les images dans l'une des deux catégories, "illustrées" ou "texte seul". Bien que nous puissions créer un modèle qui classe les images dans une parmi de nombreuses catégories, une autre approche consiste à utiliser un modèle qui attribue des étiquettes aux images. Grâce à cette approche, une image peut être associée à une seule étiquette, à plusieurs étiquettes ou à aucune étiquette. Pour l'ensemble de données sur lequel nous travaillons actuellement (les images du projet Newspaper Navigator qui ont été prédites comme étant des photos), les images ont été étiquetées plutôt que classées. Ces annotations d'étiquettes ont été créées par l'un des auteurs de la leçon. Vous pouvez trouver ce jeu de données sur [Zenodo](https://doi.org/10.5281/zenodo.4487141).

Selon la manière dont vous souhaitez appliquer la vision par ordinateur, un modèle qui effectue la classification en attribuant des étiquettes peut être plus approprié. Les données avec lesquelles vous travaillez détermineront également en partie s'il est possible d'assigner des images à une seule catégorie ou non. Comme nous l'avons vu, il a été relativement facile de classer les publicités dans les catégories "illustrées" et "non illustrées". Mais nous avons rencontré quelques "cas particuliers", par exemple les publicités contenant des [manicules](https://perma.cc/EB9D-GFE2), qui pouvent être considérées comme une forme de typographie et donc pas comme une illustration. Toutefois, il ne serait pas déraisonnable d'affirmer que les manicules jouent un rôle intentionnel &mdash;ou réel&mdash ; dans la communication de l'information, différent de celui des autres typographies, et qu'elles devraient donc être classées dans la catégorie des illustrations. Même dans cet exemple de classification relativement simple, nous commençons à voir les limites potentielles de l'approche par classification.

Les modèles qui attribuent des étiquettes au lieu d'effectuer des classifications présentent certains avantages à cet égard, car ces étiquettes préétablies peuvent fonctionner indépendamment les unes des autres. Lorsqu'on utilise un modèle de classification, une image sera toujours "poussée" dans une (et une seule) des catégories possibles (par exemple, une publicité avec ou sans illustration).  En revanche, un modèle qui applique des étiquettes peut attribuer l'étiquette $a$ sans exclure la possibilité d'attribuer également l'étiquette $b$. Un modèle qui attribue des étiquettes peut également choisir "Je ne sais pas" ou "Aucune de ces réponses", en n'attribuant aucune étiquette. Les modèles qui attribuent des étiquettes présentent également des inconvénients potentiels. L'un d'entre eux est que le processus d'annotation peut prendre plus de temps. La complexité et la rapidité avec lesquelles vous pouvez annoter les données sont un facteur important si vous allez étiqueter vos propres données, comme c'est souvent le cas dans le domaine des sciences humaines où les jeux de données prêts à l'emploi sont moins disponibles.

Nous pouvons utiliser une analogie pour illustrer la différence entre ces deux approches. Supposons que vous soyez en train de trier de vieilles photos de famille. Vous pourriez "classer" les photos dans l'un (et un seul) des deux albums photo, selon qu'elles sont en noir et blanc ou en couleur.  Cela serait comparable à l'utilisation d'un modèle de classification puisque chaque photo sera classée dans un seul de ces deux albums - une photo ne peut pas être à la fois en couleur *et* en noir et blanc, et elle ne peut être ni en couleur *ni* en noir et blanc.

Vous pouvez également vouloir faciliter la recherche des photos de certaines personnes de votre famille. Pour ce faire, vous pouvez attribuer des étiquettes à chaque photo, en indiquant ou en étiquetant les membres de la famille qui apparaissent sur la photo. Dans ce cas, une photo peut avoir une étiquette (une photo de votre sœur), plusieurs étiquettes (une photo de votre sœur *et* de votre tante) ou aucune étiquette (une photo d'un paysage prise pendant les vacances). Cette situation est analogue à celle de notre modèle de classification multi-étiquettes. 
